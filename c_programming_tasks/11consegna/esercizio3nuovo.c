#include<stdio.h>
#include<stdlib.h>
struct smazzo//struttura per gestire il mazzo
{
	int valori[52][2];
	int lung;
};
struct sgiocatore//struttura per gestire il giocatore
{
	int valori[5][2];
	float europeruno;
	float puntata;
...
int giocatori()//funzione che restituisce il numero di giocatori della partita
{
	int n=0;
...
void initmazzo(struct smazzo*maz)//inizializza le carte del mazzo
{
	int k=0;
	int y=1;
	int z=0;
...
void stampaplayer(struct sgiocatore*players,int pos)//stampa la mano di un giocatore
{
	int k=0;
		printf("\n\t\t\t\tgiocatore %d\n\n",pos+1);
		puts("");
...
void cambia_carte(struct smazzo*maz,struct sgiocatore**players,int posgioc)//cambia le carte di un giocatore, prende le nuove carte da sopra il mazzo, ripone le scartate in fondo
{
	int n=0;
	int k=0;
	int z=0;
...
void stampa(struct smazzo maz)//stampa il mazzo
{
	int k=0;
...
void mescolatore(struct smazzo*maz)//mescola il mazzo
{
	int k=0;
	int y=0;
	int z=0;
...
void distributore(struct smazzo*maz,struct sgiocatore**players,int num)//distribuisce le carte ai giocatori da sopra il mazzo
{
	int k=0;
	int z=0;
	int c=0;
...
void abbandono(struct smazzo*maz,struct sgiocatore**players,int posgioc)//funzione che gestisce l'abbandono di un giocatore dalla mano
{
	int k=0;
	for(k=0;k<5;k++)
	{
...
float puntata(struct sgiocatore**players,int posgioc)//puntata di un giocatore
{
	float puntata=0;
...
float puntmax(struct sgiocatore*players,int num)//restituisce la puntata massima dei giocatori
{
	float puntmax=0;
	int k=0;
...
int giocmax(struct sgiocatore*players,int num,float puntmax)//restituisce la posizione del giocatore che ha eseguito la puntata massima
{
	int k=0;
	for(k=0;k<num;k++)//trovo la puntata massima
	{
...
void vit_per_punti(struct sgiocatore**players,int num,float vinc)//funzione che gestisce la vittoria per punti
{
	int k=0;
	int maxscore=0;
	int vincitore=0;
...
void vit_per_abbandono(struct sgiocatore**players,int num,float vinc_iniz)//funzione che gestisce la vittoria per abbandono
{
	int k=0;
	int vincitore=0;
	float vincita=0;
...
void ord_carte(struct sgiocatore** players,int num)//funzione che ordina le carte dei giocatori
{
	int k=0;
	int z=0;
	int x=0;
...
void punteggio(struct sgiocatore**players,int num)//funzione che calcola il punteggio di ogni singolo giocatore
{
	int k=0;
	int z=0;
	int x=0;
...
int main()
{
	...
	float denaro=0;
	float cheap=0;
	struct sgiocatore*players;//puntatore alla struttura che gestisce i giocatori
	struct smazzo mazzo;//nome di variabile del tipo struct smazzo
	int scelta=0;
	float vincita=0;
	int z=0;
	int c=0;
	...
	for(k=0;k<n;k++)//primo turno di puntata
	{
		if(players[k].valori[0][0]!=-1)
		{
			printf("\t");
	...
	if(c<n-1)
	{
		...
		if(c<n-1)
		{
			...
			punteggio(&players,n);//conteggio dei punteggi
			for(k=0;k<n;k++)
			{
				if(players[k].valori[0][0]!=-1)
				{
					...
					{
						printf("il giocatore %d non ha fatto alcuna combinazione\n",k+1);
					}
					if(players[k].punteggio>=100&&players[k].punteggio<200)
					{
						printf("il giocatore %d ha fatto coppia\n",k+1);
					}
					if(players[k].punteggio>=200&&players[k].punteggio<300)
					{
						printf("il giocatore %d ha fatto doppia coppia\n",k+1);
					}
					if(players[k].punteggio>=300&&players[k].punteggio<400)
					{
						printf("il giocatore %d ha fatto tris\n",k+1);
					}
					if(players[k].punteggio>=400&&players[k].punteggio<500)
					{
						printf("il giocatore %d ha fatto scala\n",k+1);
					}
					if(players[k].punteggio>=500&&players[k].punteggio<600)
					{
						printf("il giocatore %d ha fatto full\n",k+1);
					}
					if(players[k].punteggio>=600&&players[k].punteggio<700)
					{
						printf("il giocatore %d ha fatto colore\n",k+1);
					}
					if(players[k].punteggio>=700)
					{
						printf("il giocatore %d ha fatto poker\n",k+1);
					}
				}
			}
			vit_per_punti(&players,n,vincita);//determinazione del vincitore
		}
	}
	free(players);
	return 0;
